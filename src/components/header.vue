<template>
  <header class="header">
    ...
    <div class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':loginModel}">
    ...
    <div class="md-overlay" v-show="loginModel" @click="closeModel()"></div>
  </header>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      login() {
        if (this.userName.length < 2 || this.userPwd.length < 1) {
          this.errorTip = true
          return false;
        }
