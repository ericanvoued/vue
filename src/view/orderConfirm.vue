              ...
              <li v-for="item in carList" v-if="item.checked=='1'">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img v-bind:src="'/static/'+item.imgUrl">
                  </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{(parseInt(item.productNum)*parseFloat(item.price)) | currency('$')}}</div>
                </div>
              </li>
            </ul>
          </div>
        ...
        <div class="price-count-wrap">
          <div class="price-count">
            <ul>
              ...
              <li>
                <span>Discount:</span>
                <span>{{discountPer*subtotal | currency('$')}}</span>
              </li>
              <li>
                <span>Tax:</span>
                <span>{{subtotal*taxPer | currency('$')}}</span>
              </li>
              <li class="order-total-price">
                <span>Order total:</span>
                <span>{{subtotal+shipping-discountPer*subtotal+subtotal*taxPer | currency('$')}}</span>
              </li>
            </ul>
          </div>
        </div>
...
<script>
  ...
  export default{
    ...
    methods:{
      init(){
        axios.get('/users/carList').then(data=>{
          this.carList = data.data.result;
          this.carList.map(item=>{
            if(item.checked=='1'){
              this.subtotal += parseFloat(item.price)+parseInt(item.productNum)
            }
          })
        })
      },
      payment(){
        this.orderTotal = this.subtotal+this.shipping-this.discountPer*this.subtotal+this.subtotal*this.taxPer;
        axios.post('/users/payment',{
          ...
          if(data.data.status=='1'){
            this.$router.push({
              path:'/orderSuccess?orderId='+data.data.result.orderId + '&orderTotal='+this.orderTotal
            })
          }
        })
      }
