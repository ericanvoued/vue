<template>
  <div>
              ...
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked=='1'}" @click="editNum('',item)">
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{parseInt(item.price) | currency("$")}}</div>
                </div>
    ...
    <model :mdShow = "modalConfirm" @close = 'closeModel()'>
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delComfirm()">确认</a>
  ...
  export default{
    ...
    computed:{
      ...
      totlePrice(){
        ...
        this.cartList.map(item=>{
          if(item.checked=='1'){
            maney += parseFloat(item.price*item.productNum)
          }
        })
        return maney;
      }
    ...
    methods: {
      init(){
        axios.get('/users/carList').then(data => {
          this.cartList = data.data.result;
        })
      },
      delComfirm(){
        axios.post('/users/carList/del',{productId:this.productId}).then(data=>{
      ...
      // toggleCheck(item) {
      //   item.checked = (item.checked == '1' ? '0' : '1');
      //   axios.post('/users/cartEdit/toggleCheck', {checked: item.checked}).then(data => {
      //     console.log(data)
      //   })
      // }
      toggleCheckAll(){
        ...
        axios.post('/users/carList/checkAll',{
          checkAll:flag
        }).then(data=>{
          console.log(data)
        })
